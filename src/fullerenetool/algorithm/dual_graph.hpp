...
namespace dual_graph
{
  ...
  public:
    ...
    graph_circle_finder(const int edge_num, const long *edge_origin)
    {
      ...
      property_map<dual_graph::graph, edge_index_t>::type e_index = get(edge_index, g);
      graph_traits<dual_graph::graph>::edges_size_type edge_count = 0;
      graph_traits<dual_graph::graph>::edge_iterator ei, ei_end;
      for (boost::tie(ei, ei_end) = edges(g); ei != ei_end; ++ei)
        put(e_index, *ei, edge_count++);
      //Check planarity.
      typedef std::vector<graph_traits<dual_graph::graph>::edge_descriptor> vec_t;
