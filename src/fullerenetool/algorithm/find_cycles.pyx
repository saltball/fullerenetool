...
cdef extern from "find_cycles.hpp" namespace "cycle_finder":
    cdef cppclass graph_cycle_finder:
        graph_cycle_finder(int edge_num, const long* edge_origin)
        vector[vector[unsigned long]] find_and_print_cycles()

cdef class py_graph_cycle_finder:
    cdef graph_cycle_finder* c_finder

    def __cinit__(self, int edge_num, np.ndarray[long, ndim=2] edge_origin):
        """
        ...
        # Create the C++ object
        self.c_finder = new graph_cycle_finder(edge_num, <long*> edge_origin.data)
