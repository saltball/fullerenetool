...
class DPatomic(Calculator):
    """Implementation of ASE deepmd calculator with atomic energy output

    Modified from deepmd-kit package
    ...
    def calculate(
        ...
        if self.atomic:
            e, f, v, e_a, v_a = self.dp.eval(
                coords=coord, cells=cell, atom_types=atype, atomic=True
            )
        else:
        ...
        if self.atomic:
            self.results["atomic_energy"] = e_a
        else:
            self.results["atomic_energy"] = None
