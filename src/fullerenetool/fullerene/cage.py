...
class BaseAbstactGraph(ABC):
    ...
    def generate_atoms(
        ...
        Args:
            ...
            algorithm (str): The algorithm to use to generate the atoms. Supported
                algorithms: "b2n2", "b2n2thenb2",
                    - b2n2: The atoms are generated by bond-to-nucleus-to-nucleus
                        algorithm.
                    - b2n2thenb2: The atoms are generated by b2n2 and then only use the
                        bond information without the repulsion energy.
                    Defaults to "b2n2thenb2".
        ...
        if (
            ...
            and len(elements) != len(init_pos)
        ):
            raise ValueError(
                "The number of elements must be equal to the number of init_pos,"
                " got {} and {}".format(len(elements), len(init_pos))
            )

        if algorithm in ["b2n2", "b2n2thenb2"]:
        ...
        if algorithm == "b2n2thenb2":
            atoms.calc = BondTopoBuilderCalculator(
                topo=nx.adjacency_matrix(self.graph).todense(),
                device="cuda" if use_gpu else "cpu",
                no_repulsion=True,
            )
...
class CageGraph(BaseAbstactGraph):
    ...
    def circle_matrix(self):
        return self._get_circle_matrix()
...
class FullereneCage(BaseAbstartFullerene):
    ...
    def graph(self, pick_atom_numbers=None) -> CageGraph:
        ...
        graph = get_graph_from_atoms(picked_atoms, only_top3=True)
    ...
    @property
    def name(self) -> str:
        return "Cage_{}".format(self.atoms.get_chemical_formula())
